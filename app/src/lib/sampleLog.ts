...
export function downloadSampleLogs(): void {
  ...
  const files: Record<string, Uint8Array> = {
    [`access-${isoDateTime(now - 3 * day)}.log`]: enc.encode(generateSampleLog(200, now - 3 * day, now - 2 * day)),
    [`access-${isoDateTime(now - 2 * day)}.log`]: enc.encode(generateSampleLog(250, now - 2 * day, now - 1 * day)),
    [`access-${isoDateTime(now - 1 * day)}.log`]: enc.encode(generateSampleLog(300, now - 1 * day, now)),
  }
